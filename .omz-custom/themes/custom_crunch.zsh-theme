...
if [[ -s "$HOME/.nvm/nvm.sh" ]]; then
    CRUNCH_NODE_="$CRUNCH_BRACKET_COLOR"["${CRUNCH_NODE_COLOR}node \${\$(node --version)}$CRUNCH_BRACKET_COLOR"]"%{$reset_color%}"
    CRUNCH_NPM_="$CRUNCH_BRACKET_COLOR"["${CRUNCH_NODE_COLOR}npm \${\$(npm --version)}$CRUNCH_BRACKET_COLOR"]"%{$reset_color%}"
fi
...
# Put it all together!
PROMPT="$CRUNCH_EX_$CRUNCH_RB_$CRUNCH_NODE_$CRUNCH_NPM_$CRUNCH_STATE_$CRUNCH_DIR_$CRUNCH_PROMPT%{$reset_color%}"
