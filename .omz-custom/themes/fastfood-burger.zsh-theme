...
if [ -e ~/.rvm/bin/rvm-prompt ]; then
  FASTFOOD_RVM_="$FASTFOOD_BRACKET_COLOR"["$FASTFOOD_RVM_COLOR\${\$(~/.rvm/bin/rvm-prompt i v g)#ruby-}$FASTFOOD_BRACKET_COLOR"]"%{$reset_color%}"
else
  if which rbenv &> /dev/null; then
    FASTFOOD_RVM_="$FASTFOOD_BRACKET_COLOR"["$FASTFOOD_RVM_COLOR\${\$(rbenv version | sed -e 's/ (set.*$//' -e 's/^ruby-//')}$FASTFOOD_BRACKET_COLOR"]"%{$reset_color%}"
  fi
fi
...
# Put it all together!
PROMPT="$DALLAS_CURRENT_MACH_$FASTFOOD_RVM_$FASTFOOD_DIR_$FASTFOOD_PROMPT%{$reset_color%}"
